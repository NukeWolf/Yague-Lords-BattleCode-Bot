...
public strictfp class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            if(turnCount % targetRefreshRate == 0 || me.distanceSquaredTo(randomTarget) <= myVision){
            ...
            try {
                ...
                switch (rc.getType()) {
                    ...
                    case WATCHTOWER: runWatchtower(rc);
                    case BUILDER: runBuilder(rc);
                    case SAGE:       break;
                }
            } catch (GameActionException e) {
    ...
    static void runMiner(RobotController rc) throws GameActionException {
        ...
        //Bot stats
        exploreFactor = 0.05; //how much the robot will want to explore
        exploreSearchDistance = 5;
        distanceFactor = 0.1; //how much the robot cares about getting closer to its goal
        avoidSameTypeFactor = 5; //how much the robot avoid other robots of its type
        retreatFactor = 4; //how much the robot cares about moving away from known enemies (based on damage)
        rubbleFactor = 0.2; //how much robot cares about not stepping on rubble
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (bots.length > 0) {
            ...
            RobotType[] attackPriority = {RobotType.ARCHON, RobotType.SOLDIER, RobotType.LABORATORY, RobotType.BUILDER, RobotType.MINER, RobotType.SAGE, RobotType.WATCHTOWER};
            int bestTarget = Integer.MAX_VALUE;

            for (int i = 0; i < bots.length; i++) {
                if(bots[i].team == opponent) {
                    int pIndex = Calculators.indexOfArrayValue(attackPriority, bots[i].type);
                    if (bestTarget > pIndex) {
                        bestTarget = i;
                        toAttack = bots[i].location;
                    }
                }
            }
        ...
        RobotType[] guardPriority = {RobotType.MINER, RobotType.LABORATORY, RobotType.BUILDER, RobotType.SAGE, RobotType.WATCHTOWER};
        int bestGuardTargetIndex = Integer.MAX_VALUE;

        //Bot stats
        exploreFactor = 0.1; //how much the robot will want to explore
        exploreSearchDistance = 5;
        distanceFactor = 0.1; //how much the robot cares about getting closer to its goal
        avoidSameTypeFactor = 0; //how much the robot avoid other robots of its type
        retreatFactor = 0.2; //how much the robot cares about moving away from known enemies (based on damage)
        rubbleFactor = 0.3; //how much robot cares about not stepping on rubble


        for (int i = 0; i < bots.length; i++) {
            if (bots[i].team == myTeam) {
                int pIndex = Calculators.indexOfArrayValue(guardPriority, bots[i].type);
                if (bestGuardTargetIndex > pIndex) {
                    bestGuardTargetIndex = i;
                    toMove = bots[i].location;
                }
            }
        }

        //Retreat Code
        //why retreat? - outnumbered or will die

        // Also try to move randomly.
        Direction dir = null;
        if (toMove != null){
            dir = efficientPath(rc, toMove);
        }else{
            dir = efficientPath(rc, randomTarget);
        }
    ...
    static void runBuilder(RobotController rc) throws GameActionException {
        ...
        //Repair Watchtowers
        RobotInfo[] nearbyRobots = rc.senseNearbyRobots(myAction);

        for(RobotInfo robot : nearbyRobots){
            if((robot.type == RobotType.WATCHTOWER || robot.type == RobotType.LABORATORY|| robot.type == RobotType.ARCHON) && robot.team == rc.getTeam()){
                //System.out.println(rc.canRepair(robot.location));
                while (rc.canRepair(robot.location)){
                    rc.repair(robot.location);
                    rc.setIndicatorString("Repairing Watchtower" + robot.toString());
                }

                while (rc.canMutate(robot.location)){
        ...
        boolean wt_nearby = false;
        boolean lab_nearby = false;

        for(RobotInfo robot: nearbyRobots){
            if(robot.type == RobotType.WATCHTOWER){
                wt_nearby = true;
            }else if(robot.type == RobotType.LABORATORY){
                lab_nearby = true;
            }
        }
        ...
        if(!wt_nearby) {
            ...
            Direction buildDir = directions[0];
            MapLocation buildLoc = me.add(buildDir);
            //System.out.println(buildLoc.x + " " + buildLoc.y);
            int i = 0;

            while (!(buildLoc.x >= 0 && buildLoc.y >= 0 && buildLoc.x < rubbleMap.length && buildLoc.y < rubbleMap[0].length)) {

                buildDir = directions[rng.nextInt(directions.length)];
                buildLoc = me.add(buildDir);
                i++;
                if (i > 10) {
                    break;
                }
            ...
            /*
            if(!lab_nearby && rc.canBuildRobot(RobotType.LABORATORY, buildDir)){
        ...
        exploreFactor = 0; //how much the robot will want to explore
        exploreSearchDistance = 1;
        distanceFactor = 0; //how much the robot cares about getting closer to its goal
        avoidSameTypeFactor = 0.5; //how much the robot avoid other robots of its type
        retreatFactor = 0.3; //how much the robot cares about moving away from known enemies (based on damage)
        rubbleFactor = 0.3; //how much robot cares about not stepping on rubble

        Direction dir = efficientPath(rc, randomTarget);
        if (rc.canMove(dir)) {
            if(rc.senseRobotAtLocation(rc.getLocation().add(dir)) == null)
            rc.move(dir);
        }
    ...
    static void runWatchtower(RobotController rc) throws GameActionException {
        ...
        //Build Watchtowers
        MapLocation me = rc.getLocation();
        RobotInfo[] enemies = rc.senseNearbyRobots(myAction, rc.getTeam().opponent());

        for(int index = 0; index < enemies.length; index++){
            if(rc.canAttack(enemies[index].location)){
                rc.attack(enemies[index].location);
            }
        }
